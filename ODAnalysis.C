...
Bool_t ODAnalysis::Process(Long64_t entry)
{
  ...
  GetEntry(entry);
  if (entry%1000==0) cout << "Processing "<<entry<<endl;
  nentries++;
  //----------------------------------
  ...
  //other variables #BP check if times are properly ordered and are evaluated the way they should be
  ...
  for (int p=0; p<e.tpcLG.nPulses; p++) {
    ...
    for(int c = 0; c < (int) e.tpcLG.chPulseArea[p].size(); c++) {
  ...
  for (int p=0; p<e.odLG.nPulses; p++) {
    ...
    for(int c = 0; c < (int) e.odLG.chPulseArea[p].size(); c++) {
  ...
  for (int p=0; p<e.odHG.nPulses; p++) {
    ...
    for(int c = 0; c < (int) e.odHG.chPulseArea[p].size(); c++) {
